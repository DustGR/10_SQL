...
/*2a. 2a. You need to find the ID number, first name, and last name of an actor, of whom you know only the first name, "Joe." 
...
and use the data type BLOB (Make sure to research the type BLOB, as the 
...
SELECT COUNT(*) AS 'Rental Count', s.first_name, s.last_name
	FROM payment AS p
...
SELECT 
    COUNT(*) AS 'Copies of HUNCHBACK IMPOSSIBLE'
	FROM inventory
	WHERE film_id = (
...
SELECT f.title AS 'Family Films' FROM film AS f
...
SELECT f.title, COUNT(*) AS 'Rentals' FROM film AS f
    ...
    JOIN rental AS r ON i.inventory_id = r.rental_id
    GROUP BY f.film_id
    ORDER BY COUNT(*) DESC;
